...
Representative profiling was captured for a `(77, 10)` SUSP search started from
an existing `(76, 10)` SUSP with `mode = 1` and `strong = 1`. The enhanced
run was constrained to one NVIDIA GeForce RTX 5070 Ti; Nsight Systems CPU sampling
was disabled by the host configuration, so the comparison below focuses on CUDA
trace data and Nsight Compute kernel counters.

| 30-second Nsight Systems trace | Baseline staged search | Enhanced staged search |
| --- | --- | --- |
| Report | `profiler/search_ils_staged_77_10_30s_nsys.nsys-rep` | `profiler/search_ils_staged_enhance_77_10_30s_nsys.nsys-rep` |
| CUDA kernel data in first window | None recorded | 84,349 kernel instances |
| CUDA launch API calls | None recorded | 84,354 `cudaLaunchKernel` calls |
| Sum of GPU kernel durations | None recorded | 28.37 s |
| Host-to-device traffic | None recorded | 481.6 MB across 3,095 copies |
| Device-to-host traffic | None recorded | 3.24 MB across 21,509 copies |
| GPU memset traffic | None recorded | 126.5 GB across 84,182 operations |

The important result is architectural: the original staged search spends the
first target stage expanding the initial row-replacement frontier on the CPU, so
no CUDA work appears in the first 30-second timeline. The enhanced search streams
that same frontier into pinned `GpuTask` slots and immediately keeps the GPU busy
with a batched simplification pipeline.

Top enhanced kernels in the same Nsight Systems trace:

| Kernel | Instances | Total GPU Time |
| --- | ---: | ---: |
| `modify_TDM_Kernel` | 14,061 | 10.76 s |
| `calc_projection_Kernel` | 20,622 | 10.05 s |
| `modify_TDM_face2_Kernel` | 6,561 | 4.54 s |
| `fb_kern_TC_block_Kernel` | 20,622 | 1.17 s |
| `sum<256>` | 804 | 1.08 s |
| `computeTDM_smallK_cached_Kernel` | 806 | 0.73 s |

Nsight Compute adds the per-kernel story. A strict first-30-second NCU baseline
capture produced no kernels, so the baseline NCU report records the first
baseline CUDA kernel that appears after the CPU frontier expansion:
`profiler/search_ils_staged_77_10_30s.ncu-rep`. The enhanced report is
`profiler/search_ils_staged_enhance_77_10_30s.ncu-rep`.

| NCU kernel sample | Avg duration | Avg achieved occupancy | Avg SM throughput | Launch shape |
| --- | ---: | ---: | ---: | --- |
| Baseline `compute_TDM` | 22.5 us | 73.3% | 31.2% | 1,000 blocks, 512K threads |
| Enhanced `computeTDM_smallK_cached_Kernel` | 1.04 ms | 95.4% | 82.0% | 77,824 blocks, 19.9M threads |
| Enhanced `calc_projection_Kernel` | 1.23 ms | 89.8% | 29.3% | 28,980 blocks, 7.4M threads |
| Enhanced `fb_kern_TC_block_Kernel` | 107.0 us | 83.1% | 67.9% | 1,024 blocks, 262K threads |
| Enhanced `modify_TDM_Kernel` | 1.84 ms | 97.9% | 36.7% | 4,096 blocks, 1.0M threads |
| Enhanced `modify_TDM_face2_Kernel` | 2.30 ms | 97.0% | 11.6% | 4,096 blocks, 1.0M threads |

These numbers are not meant as a universal benchmark. They are a focused systems
snapshot showing what changed: the enhanced path converts the searcher from a
CPU-led loop with occasional single-puzzle CUDA calls into a sustained GPU
pipeline with large launch shapes, high achieved occupancy in the hot kernels,
and low result-copy volume.
