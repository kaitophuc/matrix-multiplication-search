 ...
 *
 * Warning: This code isn't thread safe.  densenauty() doesn't appear
 * to be thread safe.
 *
 * Author: Matt.
 */
...
void canonize_puzzle(const Puz& p, int s, int k, int n, int* lab, graph* canon_g) {
  assert(s * k <= 5000);  // nauty crashes if graph too big.
...
bool have_seen_isomorph(const Puz& p, bool remember, int* index) {
  ensure_initilize();
  int s = p.getHeight();
  int k = p.getWidth();
  int n = s * k + 3 * (1 + 4) + (s + k);
  ...
  if (bloom_filter_is_enabled()) {
    bool seen = bloom_filter_check_and_add(canon_g, graph_size, remember);
    // Note: 'index' will not be populated when using the bloom filter.
    return seen;
  }
...
void canonize_puzzle(Puz& p) {
  ...
  for (int r = 0; r < s; r++) {
    for (int c = 0; c < k; c++) {
      int e = p_copy.get(r, c);
      p.set(row_perm[r], col_perm[c], pend_perm[e - 1] + 1);
    }
  }
}
...
bool are_isomorphs(const Puz& p1, const Puz& p2) {
  assert(p1.getWidth() == p2.getWidth());
...
size_t get_num_isomorphs() {
  ensure_initilize();
  if (bloom_filter_is_enabled()) {
    return bloom_filter_estimate_count();
  }
  return seen_isomorphs->size();
}

// Save current isomorph cache state to external variables
void save_isomorph_state(bool& ext_initilized, std::map<std::string, int>& ext_seen_isomorphs,
                         ...
                         int& ext_stored_seen) {
  ext_initilized = initilized;
  ext_num_seen = num_seen;
  ext_stored_seen = stored_seen;

  if (initilized && seen_isomorphs != nullptr) {
    ext_seen_isomorphs = *seen_isomorphs;
  } else {
    ext_seen_isomorphs.clear();
  }

  if (initilized && stored_isomorphs != nullptr) {
    ext_stored_isomorphs = *stored_isomorphs;
  } else {
    ext_stored_isomorphs.clear();
  }
}

// Restore isomorph cache state from external variables
void restore_isomorph_state(const bool& ext_initilized,
  ...
  initilized = ext_initilized;
  num_seen = ext_num_seen;
  stored_seen = ext_stored_seen;

  if (!initilized) {
    if (seen_isomorphs != nullptr) {
      seen_isomorphs->clear();
    }
    if (stored_isomorphs != nullptr) {
      stored_isomorphs->clear();
    }
  } else {
    // Ensure global pointers are allocated
    if (seen_isomorphs == nullptr) {
      seen_isomorphs = new std::map<std::string, int>();
    }
    if (stored_isomorphs == nullptr) {
      stored_isomorphs = new std::map<std::string, int>();
    }

    *seen_isomorphs = ext_seen_isomorphs;
    *stored_isomorphs = ext_stored_isomorphs;
  }
}
